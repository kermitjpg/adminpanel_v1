...
if (isset($_POST['sil']) && $_SESSION["yetki"] == "1") {
    ...
    $sorgu = $baglanti->prepare('DELETE FROM kripto WHERE id IN (' . $silinecekler . ')');
    $sorgu->execute();
    if ($sorgu) {
    ...
    Swal.fire({
        ...
        if (value.isConfirmed) {
            window.location.href = 'kripto.php'
        }
    })
</script>";
    }
...
<main>
    ...
    <div class="container-fluid px-5 py-4">
        <div class="row">
            <h2 class="text-center pt-2 pb-5">Kripto Toplam Kasa
                Tutarı: <?= /* Sorgu ahead.php'de tanımlı */
                        $kriptoTopla["tutar"] ?>₺</h2>


            <!-- EDİT -->
            <!-- Onaylanan ödemeler sayı bilgisi -->
            <?php
            $onayOkundu = $baglanti->prepare("SELECT COUNT(*) AS sayi FROM kripto WHERE aktif=1");
            $onayOkundu->execute();
            ...
            <div class="row d-flex justify-content-center">
                ...
                <?php
                $redOkundu = $baglanti->prepare("SELECT COUNT(*) AS sayi FROM kripto WHERE aktif=2");
                $redOkundu->execute();
                ...
                <div class="col-sm-2">
                    <div class="card text-white text-center bg-warning bg-gradient mb-3" style="max-width: 800px;">
                        ...
                        <div class="card-body">
                            <h5 class="card-title"><span id="okunmaSayisi" class="text-light"> <?= $sonucOkundu5["sayi"] ?></span>
                                Ödeme</h5>
                            <p class="card-text"></p>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table id="" class="dataTable">
                            ...
                            ?>
                                ...
                                <a href="#" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#silModal"><span class="fa fa-trash"></span>
                                    Tümünü
                                    Sil</a>
                                <!-- Modal -->
                            ...
                            <thead class="">
                                <tr>
                                    ...
                                    <th class="text-center">Gönderilen Coin</th>
                            ...
                            <tbody>
                                ...
                                $sorgu = $baglanti->prepare("select * from kripto order by okundu");
                                $sorgu->execute();
                                ...
                                ?>
                                    ...
                                    <tr <?php if ($sonuc["okundu"] == 0) echo 'class="fw-bold"' ?>>
                                        ...
                                        <td class="text-center"><?= $sonuc["gonderilen_coin"] ?></td>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <button class="btn btn-success btn-sm oku" type="button" onclick="change(<?= $sonuc['id'] ?>, 1);" name="onay" id="<?= $sonuc["id"] ?>">
                                                Onayla
                                            </button>
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <a href="sil.php?id=<?= $sonuc["id"] ?>&tablo=kripto" class="btn btn-danger">Sil</a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
...
if ($_POST) { ///veri güncelle
    $guncelleSorgu = $baglanti->prepare("Update kripto set aktif=:aktif WHERE id=:id");
