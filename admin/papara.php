...
if (isset($_POST['sil']) && $_SESSION["yetki"] == "1") {
    ...
    $sorgu = $baglanti->prepare('DELETE FROM papara WHERE id IN (' . $silinecekler . ')');
    $sorgu->execute();
    if ($sorgu) {
    ...
    Swal.fire({
        ...
        if (value.isConfirmed) {
            window.location.href = 'papara.php'
        }
    })
</script>";
    }
...
<main>
    ...
    <!-- <?php
            $paparadeneme = $baglanti->query("SELECT * FROM papara WHERE sistem_saati BETWEEN NOW() - INTERVAL 3 DAY AND NOW() ORDER BY sistem_saati ASC")->fetch(PDO::FETCH_ASSOC);
            ?> -->
    ...
    <div class="container-fluid px-5 py-4">
        <div class="row">
            <h2 class="text-center pt-2 pb-5">Papara Toplam Kasa
                Tutarı: <?= /* Sorgu ahead.php'de tanımlı */
                        $paparaTopla["tutar"] ?>₺</h2>
            ...
            <?php
            $onayOkundu = $baglanti->prepare("SELECT COUNT(*) AS sayi FROM papara WHERE aktif=1");
            $onayOkundu->execute();
            ...
            <div class="row d-flex justify-content-center">
                ...
                <?php
                $redOkundu = $baglanti->prepare("SELECT COUNT(*) AS sayi FROM papara WHERE aktif=2");
                $redOkundu->execute();
                ...
                <div class="col-sm-2">
                    <div class="card text-white text-center bg-warning bg-gradient mb-3" style="max-width: 800px;">
                        ...
                        <div class="card-body">
                            <h5 class="card-title"><span id="okunmaSayisi" class="text-light"> <?= $sonucOkundu2["sayi"] ?></span>
                                Ödeme</h5>
                            <p class="card-text"></p>
                        </div>
                    </div>
                        ...
                        <table id="" class="dataTable">
                            ...
                            <thead class="">
                                <tr>
                                    ...
                                    <th class="text-center">Papara Kullanıcı</th>
                            ...
                            <tbody>
                                ...
                                $sorgu = $baglanti->prepare("select * from papara order by okundu");
                                $sorgu->execute();
                                ...
                                ?>
                                    ...
                                    <tr <?php if ($sonuc["okundu"] == 0) echo 'class="fw-bold"' ?>>
                                        ...
                                        <td class="text-center"><?= $sonuc["papara_kullanici"] ?></td>
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <a href="sil.php?id=<?= $sonuc["id"] ?>&tablo=papara" class="btn btn-danger">Sil</a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
...
if ($_POST) { ///veri güncelle
    $guncelleSorgu = $baglanti->prepare("Update papara set aktif=:aktif WHERE id=:id");
